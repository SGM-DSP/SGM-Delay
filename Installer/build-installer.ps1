...
$buildOutput = Join-Path $repoRoot "build\SGMDelay_artefacts\$Configuration"
$installerOutput = Join-Path $repoRoot "build\installer"
$wxsPath = Join-Path $PSScriptRoot "Product.wxs"
$msiPath = Join-Path $installerOutput "SGM-Delay-$ProductVersion-win64.msi"

if (-not (Get-Command wix -ErrorAction SilentlyContinue)) {
    throw "WiX Toolset v4 CLI was not found. Install it with: dotnet tool install --global wix"
}

if (-not (Test-Path (Join-Path $buildOutput "Standalone\SGM Delay.exe"))) {
    throw "Standalone build output was not found. Run: cmake --build build --config $Configuration"
}

if (-not (Test-Path (Join-Path $buildOutput "VST3\SGM Delay.vst3\Contents\x86_64-win\SGM Delay.vst3"))) {
